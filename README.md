...
## 4️⃣ 기술스텍
    server side : AWS, Oracle Cloud, docker/docker-compose 
...
>: 증권사 Crawler를 통해 수집된 데이터와 등락폭이 높은 순위와 비교하여 어떤 매체가 가장 영향력을 발휘하는지 일자별로 확인하여 저장한다.
>: 1년치 데이터를 기준으로 확인하였을때 가장 영향력있는 매체를 선별한다.   
>: 현재 적용되어 있는 모델 : 선형회귀, RF, xgboost, lgbm, LSTM
...
**✔️ Infra**  
- [ ] MSA를 위한 DB 분리 및 Hadoop ecosystem 적용 
